...
async function createSession(env: Env) {
  ...
  const token = await sha256(`${id}:${env.SESSION_SECRET}:${crypto.randomUUID()}`);
...
function getSessionToken(req: Request) {
  ...
  const match = raw.match(/(?:^|;\s*)sulan_admin=([^;]+)/);
...
async function isAuth(req: Request, env: Env) {
  ...
  const session = await env.ADMIN_KV.get(`session:${token}`, "json");
  return !!session;
}

async function clearSession(req: Request, env: Env) {
  const token = getSessionToken(req);
      ...
      if (req.method === "POST") {
        ...
        if (user === env.ADMIN_USER && pass === env.ADMIN_PASSWORD) {
          return new Response(null, {
            ...
            headers: {
              Location: "/admin/",
              "Set-Cookie": cookie("sulan_admin", await createSession(env), 86400)
            }
          });
        }
